...
func BenchmarkGameStateCanTakeDiscardNowOpenedAddition(b *testing.B) {
	...
	state := benchmarkDiscardState(
		[]Card{
			card(Jack, Hearts), card(Two, Clubs), card(Three, Diamonds), card(Four, Spades),
			card(Five, Clubs), card(Six, Diamonds), card(Eight, Spades), card(Nine, Clubs),
			card(Jack, Diamonds), card(Queen, Spades), card(King, Clubs), card(Ace, Diamonds),
		},
		card(Ten, Hearts), true, baseRun,
	)
	b.ReportAllocs()
	b.ResetTimer()
	for b.Loop() {
...
func BenchmarkGameStateCanTakeDiscardNowOpeningAtForty(b *testing.B) {
	...
	state := benchmarkDiscardState(
		[]Card{
			card(King, Hearts), card(King, Diamonds), card(King, Clubs), card(Two, Spades),
			card(Three, Clubs), card(Four, Diamonds), card(Five, Spades), card(Six, Clubs),
			card(Seven, Diamonds), card(Eight, Spades), card(Nine, Clubs), card(Jack, Diamonds),
		},
		card(Ten, Hearts), false, baseRun,
	)
	b.ReportAllocs()
	b.ResetTimer()
	for b.Loop() {
